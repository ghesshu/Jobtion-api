...
const signupOTP = async (req, res) => {
  try {
    ...
    const checkUserOTP = await executeQuery({
      query: `SELECT email, phone_number FROM o_t_p_s WHERE email = ? AND phone_number = ?`,
    ...
    if (checkUserOTP.length > 0) {
      ...
      const deleteUserOTP = await executeQuery({
        ...
        values: [checkUserOTP[0].email, checkUserOTP[0].phone_number],
      });
...
const generate = async (req, res) => {
  try {
    ...
    const checkUser = await executeQuery({
      query: `SELECT email, phone_number FROM users WHERE id = ?`,
      // values: [email, phone_number],
      values: [res.locals.user_id.id, phone_number],
    });
    ...
    if (checkUser.length > 0) {
      ...
      const checkUserOTP = await executeQuery({
        query: `SELECT email, phone_number FROM o_t_p_s WHERE email = ? AND phone_number = ?`,
        values: [checkUser[0].email, checkUser[0].phone_number],
      });

      if (checkUserOTP.length > 0) {
        //*****delete & recreate */
        const deleteUserOTP = await executeQuery({
          query: `DELETE FROM o_t_p_s WHERE email = ? AND phone_number = ?`,
          values: [checkUser[0].email, checkUser[0].phone_number],
        });
      ...
      logger("otp").error("No User Found For  : ", checkUser[0].email);
      // return false;
      res.status(422).json(
        Response({
          success: false,
...
const verify = async (req, res) => {
  try {
    ...
    if (user.length > 0) {
      //*****Check of user otp already exits */
      const checkUserOTP = await executeQuery({
        query: `SELECT * FROM o_t_p_s WHERE email = ?`,
        values: [user[0].email],
      });
