...
const fetchCandidates = async (req, res) => {
  try {
    const Response_ = await axios.get(`${res.locals.uzziel}candidates`, {
      headers: {
...
const fetchSingleClient = async (req, res) => {
  try {
    const Response_ = await axios.get(`${res.locals.uzziel}clients/${req.body.id}`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      }
    ...
    if (Response_.status === 200) {
      console.log("Success : ",Response_.data)
      ...
      res.status(422).json(
        Response({
          ...
          data: Response_.data.candidates.data
        })
      );
    }
...
const fetchSingleCandidate = async (req, res) => {
  try {
    ...
    const Response_ = await axios.get(`${res.locals.uzziel}candidates/${req.body.id}`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      }
    ...
    if (Response_.data.length === 0) {
      ...
      console.log("okay ::;",Response_.data)
      res.status(200).json(
        Response({
          success: true,
...
const verifyCandidate = async (req, res) => {
  try {
    ...
    const Response_ = await axios.put(`${res.locals.uzziel}candidates/${req.body.id}/verification`,
      {
        verification_status: req.body.verification_status// another example
      },
      {
      headers: {
        'Accept': 'application/json',
...
console.log("Response : ",Response_.status)
    if (Response_.status === 200) {
      // console.log("Error : ",Response_.data)
      return res.status(200).json(
        Response({
 ...
 * @param {Object} res - Express response object
 */
const uploadCandidateReference = async (req, res) => {
  try {
    ...
    // Validate input
    if (!id) {
      return res.status(400).json(Response({ success: false, message: "Candidate ID is required" }));
    }
    if (!file_name) {
      return res.status(400).json(Response({ success: false, message: "File name is required" }));
    }
    if (!documentFile) {
      return res.status(400).json(Response({ success: false, message: "Document file is required" }));
    }

    
    const formData = new FormData();
    formData.append('file_reference_type', file_name);
    formData.append('file_reference', documentFile.buffer, {
    ...
    const uploadUrl = `${res.locals.uzziel}candidates/${id}/references`;

    const backendResponse = await axios.post(uploadUrl, formData, {
      headers: {
        ...formData.getHeaders(),
        'Accept': 'application/json'
      }
    ...
    if (backendResponse.status === 200 || backendResponse.status === 201) {
      res.status(200).json(
        Response({
          ...
          data: backendResponse.data // Forward data from backend if needed
        })
      );
    } else {
      logger("upload_candidate_doc").error("Backend Error:", backendResponse.status, backendResponse.data);
      res.status(backendResponse.status || 500).json(
        Response({
          success: false,
          message: backendResponse.data.message || "Failed to upload document to backend",
        })
      );
    }
    ...
    console.log("Error : ",error.message)
    // logger("upload_candidate_doc").error("System Error:", error);
    // // Handle specific axios errors (like network error, 404 from backend)
    // if (error.response) {
    //     logger("upload_candidate_doc").error("Backend Response Error:", error.response.status, error.response.data);
    //     return res.status(error.response.status).json(
    //         Response({
    //             success: false,
    //             message: error.response.data.message || `Backend error: ${error.response.status}`,
    //         })
    //     );
    // } else if (error.request) {
    //     logger("upload_candidate_doc").error("No response received from backend:", error.request);
...
const uploadCandidateOther = async (req, res) => {
  try {
    const { id, file_name } = req.body;
    const documentFile = req.file;
    ...
    const uploadUrl = `${res.locals.uzziel}candidates/${id}/other_documents`;
...
const downloadFiles = async (req, res) => {
  try {
    ...
    const Response_ = await axios.get(`${res.locals.uzziel}candidates/${req.body.id}/download-files`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      }
    ...
    res.status(500).json(
      Response({
        success: false,
        message: `System Error : Let Uzziel Know`,
      })
    );
  }
};
...
const fetchReport = async (req, res) => {
  try {
    ...
    const Response_ = await axios.get(`${res.locals.uzziel}candidates/uzzy/get-report`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      }
...
const downloadReport = async (req, res) => {
  try {
    ...
    const Response_ = await axios.get(`${res.locals.uzziel}candidates/uzzy/download-report`, {
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      }
