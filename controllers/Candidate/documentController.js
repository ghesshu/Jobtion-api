...
const add = async (req, res) => {
  try {
      ...
      const result = await db.query(
        "INSERT INTO documents(user_id, qualification_type, upload_doc) VALUES (?,?,?)",
        [
      ...
      if (result.affectedRows > 0) {
        const result = await db.query(
          "SELECT qualification_type, upload_doc FROM qualifications  WHERE user_id = ? ",
          [
            res.locals.user_id.id,
          ]
        );
...
const edit = async (req, res) => {
  try {
      ...
      const result = await db.query(
        "UPDATE qualifications SET qualification_type = ?, doc = ?  WHERE id = ? AND user_id = ? ",
        [
          req.body.qualification_type,
          doc,
          req.body.id,
